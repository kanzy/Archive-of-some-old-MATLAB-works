...
function consolidPool=may23_resmap(inputPool)
...
grid on
v=axis;
axis([0.5, max(consolidPool(:,2))+0.5, -15, v(4)])
    ...
    case 1
        for i=1:size(consolidPool,1)
            if consolidPool(i,1)==consolidPool(i,2)
                ...
                p2=[-2, -2];
                plot(p1,p2,'.-', 'LineWidth',1, 'Color',Color, 'MarkerSize',15) %2013-03-05
                hold on
            else
                Color='b';
%                 width=2;
...
%                 hold on
                ...
                area([consolidPool(i,1)-0.25,consolidPool(i,2)+0.25],[-4,-4],'FaceColor',Color,'LineStyle','none');
                hold on

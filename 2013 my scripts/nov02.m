%%%nov02.m: for comparison of WB's two all-D, only keep the left panel of
%%%plot in oct28.m
...
plot(bxTimeFitSet(:,1), bxTimeFitSet(:,2), 'ro')
hold on
x=bxTimeFitSet(:,1);
y=bxTimeFitSet(:,2);
[b,stats] = robustfit(x,y);
...
plot(x,b(1)+b(2)*x,'m:','LineWidth',1)
hold on
xlabel('Experimental Mean RT (min)')
ylabel('Effective BXT (min)')

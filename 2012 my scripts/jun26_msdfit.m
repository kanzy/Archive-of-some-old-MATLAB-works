...
disp(' ')
InputDataType=2 %input('Use experimental or simulated HX data as input?(1=exp, 2=sim, 3=multiple exp): ');
...
switch InputDataType
    ...
    case 2
        flag=0 %input('Use a previously saved simulated data set? (1=yes,0=no) ');
        if flag==1
            ...
            jun26_msdfit_siminput %call msdfit_siminput.m
        end
...
switch AnalyType
    case 1
        ...
        AnalyStart=1 %input('Input the start residue number of the peptide: ');
        AnalyEnd=SimEnd %input('Input the end residue number of the peptide: ');
        AnalyCharge=msdfit_siminput_cs(1, SimEnd) %input('Input the charge state of the peptide: ');
        ...
        pepNum=1;
        jun26_msdfit_common %call msdfit_common.m
        if flag~=1
...
% disp('6: GeneticAlgo')
% disp(' ')
% Algo=4 %input('Input the number of choice: ');
% if Algo==3
...
% if Algo==3
% AnalyName=[date,'_',num2str(AnalyStart),'to',num2str(AnalyEnd),'_XN',num2str(XN),'_FL',num2str(fitLevel),'_Algo3k',num2str(k_nsp),'_BXC',num2str(flagBXC)];    
% else
% AnalyName=[date,'_',num2str(AnalyStart),'to',num2str(AnalyEnd),'_XN',num2str(XN),'_FL',num2str(fitLevel),'_Algo',num2str(Algo),'_BXC',num2str(flagBXC)];
% end

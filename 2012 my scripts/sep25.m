...
switch flag
    case 1
        flagBXC=1 %input('Want to do back exchange correction?(1=yes,0=no) ');
        if flagBXC==1
            flag=1; %input('Load a previously saved all-D BXC result? (1=yes,0=no) ');
            if flag==1
...
Dub=0.95 %input('Input the upper limit of real D% value (e.g., 1): ');
fitSettings.D00=D00;
fitSettings.Dlb=Dlb;
fitSettings.Dub=Dub;

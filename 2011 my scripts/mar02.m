...
for i=1:size(mascot1,1)
    ...
    for j=1:size(peptidesPool,1)
        if mascot1(i,1)==peptidesPool(j,1) && mascot1(i,2)==peptidesPool(j,2) && mascot1(i,3)==peptidesPool(j,3)
            n=n+1;
            plot(mascot1(i,4),peptidesPool(j,8),'b*')
            hold on
            k=1;
        end
    end
    if k==0
        plot(mascot1(i,4),1.1,'go')
        hold on
    end
end
...
for j=1:size(peptidesPool,1)
    ...
    if k==0
        plot(-5,peptidesPool(j,8),'ro')
        hold on
    end
end
v=axis;
axis([-7,v(2),-0.06,1.15])
plot([38,38],[-0.06,1.15],'g:')
hold on
plot([-7,v(2)],[0.992,0.992],'r:')
hold on
xlabel('Mascot Score')
ylabel('Sequest P Score')

...
pepSeq='EEQLTPVAEETRARLSKELQAAQARL';
Charge=3;


[peptideMass, distND, maxND, maxD]=pepinfo(pepSeq);

selectPeaks=zeros(1+maxND+maxD, 2);
for i=1:size(selectPeaks,1)
    selectPeaks(i,1)=i-1;
    selectPeaks(i,2)=data(i,4);
end


fitAlgo=1; %1=Gaussians; 2=Binomials (both just for D sample fitting)
...
%                 [cfun,gof] = fit(Xdata,Ydata,f);
    ...
    if ifPlot==1
        ...
        v=axis;
        text(v(2)*0.8, v(4)*0.9, ['bestFitAdjrsquare=', num2str(FitsResult.bestFitAdjrsquare,'%5.4f')], 'FontSize',8);
        switch FitsResult.bestFitType
            case 1
                title('Deuterated Sample Fittings (magenta=2Gauss; yellow=Gauss+allH)');
                text(v(2)*0.8, v(4)*0.8, 'bestFitType="2Gauss"','FontSize',8);
            case 2
                title('Deuterated Sample Fittings (magenta=2Gauss; yellow=Gauss+allH)');
                text(v(2)*0.8, v(4)*0.8, 'bestFitType="Gauss+allH"','FontSize',8);
            case 3
                title('Deuterated Sample Fittings (magenta=2Binomials)');
                text(v(2)*0.8, v(4)*0.8, 'bestFitType="2Binomials"','FontSize',8);
            case 5
                title('Deuterated Sample Fittings (magenta=Polynomial)');
                text(v(2)*0.8, v(4)*0.8, 'bestFitType="Polynomial"','FontSize',8);
        end
    end
    ...
    if ifPlot==1
        ...
        xlabel('delta Mass(above monoisotopic)');
        ylabel('Peak Intensity');
        title('allH Sample Fittings (yellow)');
        v=axis;
        text(v(2)*0.8, v(4)*0.9, ['FitAdjrsquare=', num2str(FitsResult.bestFitAdjrsquare,'%5.4f')], 'FontSize',8);
    end

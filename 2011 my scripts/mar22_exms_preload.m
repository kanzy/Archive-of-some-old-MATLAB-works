    ...
    case 1 %for OpenOffice converted .csv, the old default
        ...
        for i=1:numMSMS
            ...
            for j=4:size(SequestOutput,1) %first 3 rows in .csv file are skipped
                ...
                %%%2011-03-22 added:
                byString=SequestPepInfo((commaPositions(comma_P+5)+2):(commaPositions(comma_P+6)-2));
                x=find(byString=='/');
                byMatch=byString(1:(x-1));
                byMax=byString((x+1):end);
                ...
                if SequestPepPscore<=PscoreThreshold1 %filter by P score threshold
                    ...
                    if size(startPositions,2)==1 %one and only one match found along 'currSeq'
                        ...
                        %%%2011-03-22 added:
                        selectPeps(m,6)=str2double(byMatch);
                        selectPeps(m,7)=str2double(byMax);
...
%%%2011-03-22 added:
peptidesPool(1,9)=selectPeps(1,6); %byMatch
peptidesPool(1,10)=selectPeps(1,7); %byMax
...
for i=2:size(selectPeps,1)
    if (selectPeps(i,1)~=selectPeps(i-1,1) || selectPeps(i,2)~=selectPeps(i-1,2) ...
        ...
        peptidesPool(m,9)=selectPeps(i,6); %byMatch
        peptidesPool(m,10)=selectPeps(i,7); %byMax
        
    else
        if selectPeps(i,5)<minPscore
            peptidesPool(m,7)=selectPeps(i,4); %RT assigned by the best(lowest P score) peptide
            peptidesPool(m,8)=selectPeps(i,5);
            minPscore=selectPeps(i,5);
...
%         peptidesPool_isobaricTable(n,9)=i; %index in 'peptidesPool'
        ...
        peptidesPool_isobaricTable(n,1:10)=peptidesPool(i,:);
        peptidesPool_isobaricTable(n,11)=i; %index in 'peptidesPool'
...
% if flag==1 || flag==2 %for SEQUEST
    save(SaveFileName,'proteinName','currSeq','peptidesPool','peptidesPool_distND','peptidesPool_isobaricTable','theoryPool','flagTheor','PscoreThresholds')
% else %for MassLynx

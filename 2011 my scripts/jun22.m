...
AnalyEnd=50 %input('Input the end residue number of analysis range: ');
pepNum=1;
useData={};
usePeps=[];
for i=1:size(finalTable,1)
...
if size(usePeps,1)>1
    ...
    while rowNum*colNum<size(usePeps,1) %check
        ...
        rowNum=7%input('How many peptides want to be plotted in a column? '); %ask here for later auto save
        colNum=6%input('How many peptides want to be plotted in a row? ');
        n=n+1;
    end
end
